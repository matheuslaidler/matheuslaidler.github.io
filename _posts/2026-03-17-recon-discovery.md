...
Tem um mito de iniciante que diz que bug bounty é decorar payload. Não é. O caçador experiente sabe que **80% do resultado vem de onde quase ninguém olha**: aquele subdomínio esquecido de homologação, o endpoint de API que só aparece num arquivo JavaScript minificado, o `.env` que vazou num commit de três anos atrás. (Endpoint é um endereço/rota específico da aplicação que recebe requisições, tipo `/api/v1/users`. Mais no [Glossário](/posts/fundamentos-web-hacking/#glossário-rápido-os-termos-que-vão-aparecer-na-série).) O payload é o último 20%, e ele só funciona se você **achou a porta certa primeiro**.
...
O recon passivo parece mágica ("como você achou 800 subdomínios sem tocar no alvo?"), mas é só saber **quais bancos de dados públicos** registram a infraestrutura de todo mundo. Os três pilares são:
...
**subfinder** é o cavalo de batalha do recon passivo. É um enumerador que consulta **dezenas de fontes públicas** (CT logs, agregadores de DNS, motores de busca) de uma vez e te entrega a lista de subdomínios. É *passivo*: ele pergunta pras fontes, não bombardeia o alvo.
...
- `-all` → ativa todas as fontes (sem isso, usa só as gratuitas mais rápidas). **Aqui mora ouro**: o relato do bounty de cinco dígitos veio justamente de uma fonte extra. Você adiciona suas próprias chaves de API no arquivo `$HOME/.config/subfinder/provider-config.yaml` (flag `-pc`), e isso desbloqueia fontes (SecurityTrails, Censys, etc.) que pegam subdomínios que ninguém mais pega.
...
**crt.sh** é a consulta direta aos CT logs, sem instalar nada. Ele tem uma API JSON que você consome com `curl` + `jq`:
...
Cada peça: `curl -s` baixa o JSON em modo silencioso; `jq -r '.[].name_value'` percorre o array (`.[]`) e imprime o campo `name_value` (o nome do certificado) sem aspas (`-r` é raw); o `sed` limpa entradas wildcard tipo `*.alvo.com`; `sort -u` deduplica. Resultado: lista limpa de subdomínios, **100% passiva**.

**amass** é o enumerador mais completo (e mais pesado) da OWASP. A diferença pro subfinder é que o amass faz **enumeração ativa** opcional (resolução DNS, brute-force, varredura de blocos de IP), além da passiva.
...
# Ativo: técnicas que tocam o alvo + brute-force de nomes (FAZ barulho)
amass enum -active -brute -d alvo.com -o amass-active.txt
```

- `enum` → o subcomando de enumeração.
- `-passive` → só consulta fontes de terceiros, sem tocar no alvo. No amass v4 esse é o **modo padrão** (a flag virou opcional/depreciada: `amass enum -d alvo.com` já roda passivo). Comece sempre por aqui.
- `-active` → liga as técnicas **ativas** (transferência de zona DNS + captura de nomes em certificados TLS), que conversam com a infra do alvo. A resolução dos nomes encontrados o amass já faz por padrão.
...
> 💡 O LFI (*Local File Inclusion*) é ler arquivos do servidor (ex.: /etc/passwd) por parâmetros como ?file=../../etc/passwd.
{: .prompt-tip }
...
**katana (apresentando a ferramenta):** um **crawler** (rastreador) da ProjectDiscovery. Enquanto o ffuf adivinha, o katana **navega** o site como um robô, seguindo links, e (o pulo do gato) **parseia o JavaScript** pra extrair endpoints. É a ponte entre a Etapa 4 e o fuzzing.
