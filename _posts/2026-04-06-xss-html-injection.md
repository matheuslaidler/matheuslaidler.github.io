...
> ⚠️ **Cookie `HttpOnly` muda o jogo.** Se o cookie de sessão é `HttpOnly`, o `document.cookie` não enxerga ele, e aí o roubo de cookie clássico não funciona. Mas XSS continua crítico: você ainda executa ações em nome da vítima (CSRF via fetch autenticado), faz keylogger e lê dados da tela. Não desista do impacto só porque o cookie está protegido.
...
As definições são as da [PortSwigger](https://portswigger.net/web-security/cross-site-scripting): no **reflected** "o script malicioso vem da request HTTP atual"; no **stored** ele "vem do banco de dados do site"; e no **DOM-based** "a vulnerabilidade está no código client-side, não no server-side".

> 💡 **Reflected × Stored, na imagem que gruda:** o **Reflected** é entregar um **copo de água envenenado** na mão da vítima, que precisa "beber" (clicar no seu link). O **Stored** é **envenenar a água da cidade**: você contamina a fonte e **qualquer um** que beber é atingido, sem precisar convencer ninguém. Por isso o Stored costuma valer mais: escala sozinho e dispensa engenharia social.
...
> 💡 O **WAF** (Web Application Firewall) é um filtro na frente da aplicação que tenta barrar requisições maliciosas por padrões/assinaturas. Detalhe no [Glossário](/posts/fundamentos-web-hacking/#glossário-rápido-os-termos-que-vão-aparecer-na-série).
...
> - **mXSS (mutation XSS):** HTML que parece "limpo" **muda de significado** quando o navegador re-parseia (ex.: ao cair no `innerHTML`) e fura até sanitizador (o DOMPurify **desatualizado** já teve bypass por mXSS; mantenha a lib atual).
...
> 🪤 **A defesa que parece certa mas falha: `htmlspecialchars` sem `ENT_QUOTES`.** Os dois argumentos extras acima não são enfeite. Até o **PHP 8.0**, o default de `htmlspecialchars($x)` era `ENT_COMPAT`, que escapa a aspa **dupla** (`"`) mas **deixa a aspa simples (`'`) passar** (só virou `ENT_QUOTES` por padrão no PHP 8.1). Então o dev que "escapou tudo" com `htmlspecialchars($q)` num atributo de **aspas simples** continua furado:
> ```php
...
Por que funciona: `innerHTML` faz o navegador **parsear o conteúdo como HTML** (e disparar handlers); `textContent` só escreve **texto**, e o navegador nunca interpreta isso como markup. Mesma lógica vale pra fugir de `eval`, `document.write` e `setTimeout(string)`.
...
> 💡 **SSTI / CSTI** (Server-Side / Client-Side Template Injection) é injeção no motor de templates (servidor/cliente). Resultado parecido com XSS, mecanismo diferente.
{: .prompt-tip }
...
- **`X-XSS-Protection` não protege mais nada:** o header está **deprecado** e o XSS Auditor que ele controlava foi **removido** dos navegadores. Ver ele presente num alvo **não** significa que o XSS está mitigado, então ignore e teste normalmente; ver ele ausente **não** é, por si só, uma vulnerabilidade reportável.
