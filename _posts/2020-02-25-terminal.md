...
Este guia abrange desde conceitos básicos até comandos avançados do terminal Linux. Seja você iniciante ou quem precisa relembrar, aqui você encontrará:

- **Fundamentos**: O que é Linux, terminal e shell
- **Comandos essenciais**: Navegação, manipulação de arquivos, permissões
- **Filtros e pipes**: Grep, awk, sed e redirecionamento
- **Administração**: Usuários, processos, rede e sistema
- **WSL**: Como praticar no Windows
...
- **Comando**: Instrução que representa uma ou mais ações
- **Shell**: Interpretador que processa comandos (bash, zsh, fish)
- **Terminal**: Programa que acessa o shell (emulador de terminal)
...
**Dicas importantes:**
- `locate` é mais rápido mas depende de um banco atualizado (`updatedb`)
- `which` só encontra executáveis que estão no PATH
- `find` é mais lento mas busca em tempo real e tem mais opções
- Use `2>/dev/null` com find para suprimir erros de permissão
...
As variáveis de ambiente são informações armazenadas no sistema que podem ser acessadas por programas e scripts. São fundamentais para personalizar o comportamento do sistema e dos aplicativos.
...
**Dica importante:** Mudanças em ~/.bashrc só afetam novas sessões. Use `source ~/.bashrc` para aplicar imediatamente.
