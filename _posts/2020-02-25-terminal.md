...
author: matheus
tags: ["Linux", "terminal", "Bash", "linha de comando", "WSL", "shell"]
categories: ["Redes e Sistemas", "Linha de Comando"]
pin: false
comments: true
image: /assets/img/covers/terminal.png
---

## Linux: Dando início a uma fase Terminal

**Dominando linha de comando UNIX - Perfeito para quem quer conhecer, aprender, relembrar ou masterizar**

> Material focado em distribuições baseadas em Debian, como Ubuntu. Comandos funcionam na maioria das distribuições Linux com pequenas variações.

## O que tem por aqui

Aqui vai desde o básico até comandos mais avançados do terminal Linux. Seja você iniciante ou só precisando relembrar, vou passar pelos fundamentos (o que é Linux, terminal e shell), os comandos do dia a dia (navegar, mexer em arquivos, permissões), filtros e pipes (grep, sed e redirecionamento), administração (usuários, processos, rede e sistema) e como praticar pelo WSL no Windows.
...
Comando é a instrução que representa uma ou mais ações. Shell é o interpretador que processa esses comandos (bash, zsh, fish). E o terminal é o programa que te dá acesso ao shell (o tal emulador de terminal).

O **bash** é o shell mais comum e vem como padrão na maioria das distribuições.

### Linux vs GNU/Linux

**Linux** é o nome do *kernel* (núcleo) desenvolvido por Linus Torvalds. O *sistema operacional* completo são as *distribuições* como Ubuntu, Fedora, Mint, Kali. Tecnicamente deveria ser chamado **GNU/Linux**, pois combina o kernel Linux com ferramentas do projeto GNU. 

## Praticando com **Linux**
 Basta abrir o terminal e praticar. Porém, lembre-se de tomar cuidado ao usar o terminal alheio apenas copiando e colando comandos que você vê na internet, sem saber o que ele faz. A chance de dar problemas em coisas importantes são grandes. Tente ao menos entender o que está sendo feito.

**Tá, ok... tudo certo até agora, mas...**
 ... antes de irmos direto aos comandos e explicações, vamos tentar desmistificar algumas coisas sobre linux...

### Desmistificando o Pinguim

  _É comum a ideia de que “Linux é para usuários avançados” ou “para programadores”, e que é preciso dominar linhas de comando. Isso pode assustar usuários, como se fosse obrigatório. A maioria dos tutoriais na internet são feitos via terminal, o que reforça essa percepção. No entanto, não é necessário saber linhas de comando para usar o sistema no dia a dia. Tudo o que é necessário pode ser feito via interface gráfica, assim como no Windows, Mac e Android. Novos usuários de Linux podem começar com distribuições amigáveis como Zorin OS e Linux Mint. Embora não seja necessário dominar linhas de comando, é útil ter alguma noção, especialmente ao seguir tutoriais. Para usuários mais avançados e estudantes de computação, dominar o terminal é fundamental. Aqui, listei uma série de comandos para facilitar o uso do terminal no dia a dia._
...
2. Execute: `wsl --install ubuntu`
3. Reinicie o computador
4. Configure usuário e senha quando solicitado

**Instalação manual** (se o método rápido falhar):
1. `Win+R` → digite `optionalfeatures`
2. Marque "Subsistema do Windows para Linux"
3. Reinicie
4. Instale Ubuntu na Microsoft Store
5. Se der erro, execute no PowerShell como admin:

**Comandos via PowerShell como administrador**

```powershell
dism.exe /online /enable-feature /featurename:Microsoft-Windows-Subsystem-Linux /all /norestart
dism.exe /online /enable-feature /featurename:VirtualMachinePlatform /all /norestart
#Baixar a distribuição (Kali Linux, por exemplo) na Microsoft Store, reiniciar o pc.
```

**Configuração adicional para VMs**:
```powershell
bcdedit /set hypervisorlaunchtype off
```

> **⚠️ Observações importantes:**
> - Sistema deve ser 64 bits e atualizado (build 16215.0+)
> - WSL tem limitações comparado ao Linux nativo
> - Para problemas, consulte [documentação oficial da Microsoft](https://docs.microsoft.com/pt-br/windows/wsl/install-win10)
...
> *Para quem ainda não conhece direito o terminal, vamos ver o que é cada elemento da interface.*

**Exemplo de prompt**: `Matheus@Ubuntu:~$`

- ***Usuário***: Nome da conta da sessão atual (`Matheus`)
- ***PC/Host***: Nome da máquina (`Ubuntu`) 
- ***Diretório***: Pasta atual (`~` = pasta home do usuário)
- ***Modo de usuário***: `$` = usuário comum, `#` = superusuário (root)

## Principais comandos

### Ajuda e documentação

Sempre comece conhecendo estes comandos de ajuda:

```bash
man <comando>        # Manual completo
<comando> -h         # Ajuda resumida  
...
O *useradd* cria um novo usuário mas sem criar uma pasta home para o mesmo, apenas registra o usuário nos arquivos do sistema (/etc/passwd, /etc/shadow e /etc/group),  diferentemente do comando *adduser*. Porém, ambos precisam de permissão para serem executadas.
...
```
 ...
 _Com o **deluser** Também podemos retirar o usuário de algum grupo. Podemos retirar o banana do sudo_
...
shutdown -> desligar o pc  [ podendo usar para programar o desligamento: no Linux o tempo é em minutos com `+`, ex.: shutdown -h +10 (desliga em 10 minutos) ou um horário hh:mm, e dps -c para cancelar ]
...
sudo lshw -> listar hardwares (requer privilégios administrativos)
...
history # ver o histórico dos últimos comandos usados
...
```bash
# locate - localizar arquivos (mais rápido, usa banco de dados)
locate arquivo.txt          # busca qualquer arquivo com esse nome
locate "*.pdf"              # busca todos os PDFs no sistema
sudo updatedb               # atualiza o banco de dados do locate

# which - localizar um comando executável no PATH
which python3               # mostra onde está o executável python3
which firefox               # mostra o caminho do Firefox
which -a python             # mostra todas as versões de python no PATH

# whereis - localizar arquivos binários, código-fonte e manuais
whereis bash                # encontra binário, código e manual do bash
whereis -b firefox          # apenas o binário do firefox
whereis -m ls               # apenas o manual do ls

# find - procurar arquivos em tempo real (mais lento, mas mais flexível)
find /home/usuario/Documentos -name "*.txt"     # todos os .txt em Documentos
find / -name "arquivo.txt" 2>/dev/null          # busca em todo sistema, ignora erros
find . -type f -size +100M                     # arquivos maiores que 100MB
find /var/log -name "*.log" -mtime -7           # logs modificados nos últimos 7 dias
find . -name "*conf*" -type f                   # arquivos com "conf" no nome
find /home -user joao -name "*.pdf"             # PDFs do usuário joão
```

Resumindo: o `locate` é mais rápido mas depende do banco estar atualizado (`updatedb`); o `which` só acha executável que esteja no PATH; e o `find` é mais lento porém busca em tempo real e tem muito mais opção. Dica: jogue `2>/dev/null` no fim do find pra esconder os erros de permissão.
...
#    Compactar arquivos:
    ...
    tar -zcf nome_arq.tar nome_do_diretorio_do_arquivo_ou_dos_arquivos
...
Explicarei resumidamente a funcionalidade das caracteres especiais  `&&` ,  `;` ,  `&` ,  `||` ,   `|` ,  elas podem se mostrar úteis para determinadas situações. Estas não possuem o mesmo significado.
 ...
 O **`&&`** significa um "and" **(`e`)**, funciona de forma a executar o segundo comando apenas, e somente apenas, se o primeiro der um status de saída de 0 (sucesso de execução). Em outras palavras, serve para chamar um comando caso o outro não retorne erro, assim o bash emite um sinal de 0 e o operador && chama o próximo comando. Para funcionar em script, as duas precisam ser verdadeiras, como { 1>0 && 0==0 } para que os comandos dentro deste if sejam utilizados.
...
- O comando "grep b" por sua vez filtra os nomes de arquivos que contém a letra "b". Devido ao segundo pipe a saída do comando "grep b" é enviada para "sort -r", que classifica os nomes em ordem decrescente.
...
Listou os diretórios e arquivos do local, gravou a saída em 'exemplo.txt' e pediu para filtrar a saída mostrando apenas a string 'teste'. 
...
> 
> - *halt*, *telinit 0*, *init 0*, *shutdown -h now* - Desligar o sistema
>   
...
>   
>   - *nano*, *vi*, *vim*, *visudo*, *vipw*, etc  - editores de texto - criação de arquivo - *visudo* é o recomendado para editar o  arquivo *sudoers* - *vipw* para editar os arquivos _/etc/passwd_ e _/etc/shadow_.
>   
...
> 
> - *recode* - conversão de formatos de arquivos.
> 
...
As variáveis de ambiente são informações que ficam guardadas no sistema e que programas e scripts conseguem ler. Servem pra personalizar como o sistema e os apps se comportam.
...
echo $PATH        # caminhos onde o sistema procura executáveis
echo $USER        # nome do usuário atual
echo $SHELL       # shell padrão do usuário
echo $PWD         # diretório atual
echo $OLDPWD      # diretório anterior
echo $LANG        # idioma/localização do sistema

# Outras variáveis importantes
echo $EDITOR      # editor padrão
echo $BROWSER     # navegador padrão
echo $TERM        # tipo de terminal
```

### Definindo e modificando variáveis

```bash
# Definir variável temporária (só para a sessão atual)
...
# Modificar variáveis permanentemente
# Para usuário específico: ~/.bashrc ou ~/.bash_profile
echo 'export EDITOR=nano' >> ~/.bashrc
source ~/.bashrc  # reaplicar mudanças

# Para todo o sistema: /etc/environment ou /etc/bash.bashrc
```

### Comandos relacionados

```bash
which comando     # encontra localização do comando no PATH
type comando      # mostra tipo e localização do comando
set              # mostra variáveis do shell atual
unset VARIAVEL   # remove uma variável
```
...
# Usar variável em script
#!/bin/bash
BACKUP_DIR="$HOME/backups"
mkdir -p "$BACKUP_DIR"
```

Lembrando: mexer no ~/.bashrc só vale pras próximas sessões. Se quiser aplicar na hora, roda `source ~/.bashrc`.
