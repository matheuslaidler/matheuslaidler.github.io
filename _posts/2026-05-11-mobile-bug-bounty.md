...
Pronto: **o tráfego HTTP já cai no Burp**. O **HTTPS**, porém, vai dar erro de certificado, e é aí que entra o próximo passo.
...
> 💡 **Frida**: um toolkit que **injeta JavaScript dentro de um processo em execução** e "engancha" (hooks) funções em tempo real, ou seja, você consegue trocar o que uma função retorna sem recompilar o app. É o canivete suíço do mobile testing. [Glossário](/posts/fundamentos-web-hacking/#glossário-rápido-os-termos-que-vão-aparecer-na-série)
...
> ⚠️ Pinning feito direito (Flutter, implementações nativas customizadas) pode resistir ao script genérico. Aí o caminho é abrir o app no jadx, achar a função que faz o pin e escrever um **hook específico**. Mas, pra 90% dos apps, `objection ... android sslpinning disable` resolve.
...
**OWASP MASVS** (Mobile Application Security Verification Standard) é o **padrão de verificação**, o "checklist do que deve estar seguro", dividido em grupos de controle: **MASVS-STORAGE** (armazenamento), **MASVS-CRYPTO** (cripto), **MASVS-AUTH** (autn/autz), **MASVS-NETWORK** (comunicação), **MASVS-PLATFORM** (interação com a plataforma: deeplinks, WebView, IPC), **MASVS-CODE** (qualidade/dependências) e **MASVS-RESILIENCE** (resistência a engenharia reversa). O **MASTG** (Testing Guide) é o **passo a passo** de como testar cada um, a sua bíblia técnica gratuita.
